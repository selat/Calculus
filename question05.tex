...
\begin{col-answer-preambule}
  \begin{plan}
  	...
  	\item Доказательство:
    \subitem Пишем, что нужно обосновать для $\forall \; x \in X$, при этом нужно использовать односторонние пределы для концевых значений.
    \subitem Рассматриваем приращение суммы $\Delta S(x_0)$.
    \subitem Рассматриваем две разности частичной суммы и полной суммы (с $x_0$ и $x_0 + \Delta x$).
    \subitem Хитро запихиваем обе разности под модуль так, чтобы в итоге выскочило $\Delta S(x_0)$ и получаем непрерывность по M-лемме.
  \end{plan}
\end{col-answer-preambule}
